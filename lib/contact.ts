...
const CONTACT_STORAGE_FILE =
  process.env.CONTACT_STORAGE_FILE || 'contact-requests.log'

export async function persistContactRequest(
  data: ContactRequestData
): Promise<void> {
  const dir =
    process.env.CONTACT_STORAGE_DIR ||
...
export async function sendContactEmail(
  ...
  const contactEmailTo =
    process.env.CONTACT_EMAIL_TO || process.env.CONTACT_EMAIL
  const contactEmailFrom =
    process.env.CONTACT_EMAIL_FROM || process.env.CONTACT_EMAIL
